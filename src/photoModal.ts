...
export class PhotosModal extends Modal {
	...
	async insertImageIntoEditor (event: MouseEvent) {
		try {
			...
			await this.insertImage(thumbnailImage)
		} catch (e) {
			console.log(e)
		}
	...
	// New helper: insert an image object directly (no UI interaction required)
	async insertImage (thumbnailImage: any) {
		try {
			const baseUrl = thumbnailImage.baseUrl || thumbnailImage.baseUrlRaw || ''
			...
			let linkPath = thumbnailImage.filename || (thumbnailImage.photoId ? `${thumbnailImage.photoId}.jpg` : 'image.jpg')
			switch (this.plugin.settings.locationOption) {
				case 'specified':
					...
					linkPath = thumbnailFolder + '/' + linkPath
					break
				case 'subfolder':
					...
					linkPath = this.plugin.settings.locationSubfolder + '/' + linkPath
					break
			}
			...
			const linkText = handlebarParse(this.plugin.settings.thumbnailMarkdown, {
				...
				taken_date: creationMoment.format()
			})
			// Ensure there's a single trailing newline so multiple images are separated
			let finalLinkText = linkText
			if (!finalLinkText.endsWith('\n')) {
				finalLinkText += '\n'
			}
			this.editor.replaceRange(finalLinkText, cursorPosition)
			// Move the cursor to the end of the thumbnail link after pasting
			this.editor.setCursor({ line: cursorPosition.line, ch: cursorPosition.ch + finalLinkText.length })
		} catch (e) {
			console.log(e)
		}
	}
	...
	async displaySelectedPhotos () {
		...
		try {
			...
			if (mediaItemsResponse.mediaItems && mediaItemsResponse.mediaItems.length > 0) {
				...
				statusEl.setText(`Inserting ${mediaItemsResponse.mediaItems.length} photo(s) into the current note...`)
				
				// Convert picked items to compatible format
				const compatibleItems = mediaItemsResponse.mediaItems.map(item => 
					this.plugin.photosApi.convertPickedMediaItem(item)
				)
				
				// Insert each image directly into the current note
				for (const item of compatibleItems) {
					console.log('Inserting item:', item)
					await this.insertImage(item)
				}
				
				statusEl.setText('All selected photos inserted.')
				// Close modal after insertions
				this.close()
			} else {
				console.log('No media items found:', mediaItemsResponse.mediaItems)
